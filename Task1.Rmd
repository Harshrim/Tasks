...
library(car)
vif(reg1)
#Building second model by droppinf variables having no impact on house price and variables having high multicollinearity
...
```{r}
#Problem Statement 5 - 5.	Determine average number of rooms and bathrooms for each neighborhood

data1$nbh <- as.factor(data1$nbh)
data1$nbh
data2<-data1 %>% group_by(nbh) %>% summarise(mean_rooms = mean(rooms))
ggplot(aes(x = nbh, y = mean_rooms), data = data2) + geom_bar(stat = "identity")
data3<-data1 %>% group_by(nbh) %>% summarise(mean_baths = mean(baths))
ggplot(aes(x = nbh, y = mean_baths), data = data3) + geom_bar(stat = "identity")

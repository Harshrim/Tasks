...
class(data1)
unique(data1["baths"])
data4<-data1 %>% group_by(baths) %>% summarise(min_price = min(lrprice),max_price=max(lrprice))
data4

#Other Graphs
ggplot(aes(x = baths, y = min_price), data = data4) + geom_line()+geom_point()+ggtitle("Min price of house for each number of bathroom")  + theme(axis.ticks =element_blank(),
                      axis.text = element_blank(),
                      axis.title = element_blank())
```
...
```{r}
ggplot(aes(x = baths, y = max_price), data = data4) + geom_line()+geom_point()+ggtitle("Max price of house for each number of bathroom")  + theme(axis.ticks =element_blank(),
                      axis.text = element_blank(),
                      axis.title = element_blank())
```
